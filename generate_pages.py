...
with open('src/data/competitor_content.json', 'r') as f:
    data = json.load(f)

# Create a generic dynamic layout component first
layout_code = """import Link from 'next/link';
...
os.makedirs('src/components/CompetitorLayout', exist_ok=True)
with open('src/components/CompetitorLayout/DynamicServiceTemplate.tsx', 'w') as f:
    f.write(layout_code)

print("Created template component.")

for path, content in data.items():
    if path == "home": continue
    if path in ["contact-us", "about-us", "services", "our-trades", "labor-recruitment", "reviews", "blog", "privacy-policy", "terms-of-use", "faqs"]:
        # Skip some core pages that we either already have or want to build manually
        continue
        
    dir_path = os.path.join('src/app', path)
    os.makedirs(dir_path, exist_ok=True)
    
    page_code = f"""import DynamicServiceTemplate from '@/components/CompetitorLayout/DynamicServiceTemplate';
import content from '@/data/competitor_content.json';
import {{ Metadata }} from 'next';

export const metadata: Metadata = {{
  title: `{content.get('title', 'Constructech Estimation')}`,
}};

export default function Page() {{
  const data = (content as any)['{path}'];
